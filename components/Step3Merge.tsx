  ...
  const handleDownload = async () => {
    ...
    try {
        ...
        bgImages.forEach((bgSlide, index) => {
             ...
             // Handle multiple pages if data matches index
             const pageTextData = (index === 0) ? textData : (textData?.[index] || null); 
             ...
             if (pageTextData && pageTextData.textBlocks) {
                 pageTextData.textBlocks.forEach((block: any) => {
                    ...
                    slide.addText(block.text, {
                        ...
                        bold: !!block.isBold,
                        fontFace: /[\u4e00-\u9fa5]/.test(block.text) ? "Microsoft JhengHei" : "Arial",
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">最終階段：將動態文字座標覆蓋於還原底圖，生成最終可編輯 PPT。</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
        <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 text-center">
            <h3 className="font-bold text-gray-800 text-sm mb-1">文字層數據</h3>
            <div className="text-[10px] text-green-600 font-bold bg-green-100 px-2 py-1 rounded inline-block">已就緒</div>
        </div>
        ...
        <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 text-center">
            ...
            <div className="text-[10px] text-green-600 font-bold bg-green-100 px-2 py-1 rounded inline-block">已還原 (16:9)</div>
        </div>
      </div>
    </div>
  );
